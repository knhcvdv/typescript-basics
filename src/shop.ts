...
type Clothing = BaseProduct & {
    category: 'clothing';
    size: string; // Розмір (S, M, L, XL тощо)
    color: string;
    material: string;
};
...
const filterByPrice = <T extends BaseProduct>(products: T[], maxPrice: number): T[] => {
    ...
    return products.filter((product: T) => product.price <= maxPrice && product.inStock);
};
 ...
 * @returns масив товарів, що відповідають критерію
 */
const filterByMinPrice = <T extends BaseProduct>(products: T[], minPrice: number): T[] => {
    if (!Array.isArray(products)) {
        return [];
    }
    ...
    return products.filter((product: T) => product.price >= minPrice && product.inStock);
};
 ...
 * @returns масив товарів, що відповідають критерію
 */
const filterByPriceRange = <T extends BaseProduct>(
    products: T[],
    minPrice: number,
    maxPrice: number
): T[] => {
    ...
    return products.filter(
        (product: T) => product.price >= minPrice && product.price <= maxPrice && product.inStock
    );
};
...
const addToCart = <T extends BaseProduct>(
    ...
    const existingItemIndex: number = cart.findIndex(
        (item: CartItem<T>) => item.product.id === product.id
    );
 ...
 * @returns оновлений кошик
 */
const updateCartQuantity = <T extends BaseProduct>(
    cart: CartItem<T>[],
    productId: number,
    quantity: number
): CartItem<T>[] => {
    ...
    return cart.map((item: CartItem<T>) =>
...
const calculateTotal = <T extends BaseProduct>(cart: CartItem<T>[]): number => {
    ...
    return cart.reduce((total: number, item: CartItem<T>) => {
 ...
 */
const getTotalQuantity = <T extends BaseProduct>(cart: CartItem<T>[]): number => {
    if (!Array.isArray(cart) || cart.length === 0) {
        return 0;
    }
    
    return cart.reduce((total: number, item: CartItem<T>) => total + item.quantity, 0);
};
...
const totalQuantity: number = getTotalQuantity(cart);
...
const newTotal: number = calculateTotal(cart);
...
if (book) {
    ...
    const finalTotal: number = calculateTotal(cart);
